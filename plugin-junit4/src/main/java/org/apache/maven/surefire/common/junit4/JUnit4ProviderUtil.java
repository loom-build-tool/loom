...
public final class JUnit4ProviderUtil
{
     ...
     */
    public static Map<Class<?>, Set<String>> generateFailingTests( final List<Failure> allFailures,
                                                                   final ClassLoader testClassLoader )
        throws TestSetFailedException
    {
        final Map<Class<?>, Set<String>> testClassMethods = new HashMap<Class<?>, Set<String>>();
        final Set<ClassMethod> failingTests = generateFailingTests( allFailures );
        for ( final ClassMethod classMethod: failingTests )
        {
            try
            {
                final Class testClassObj = Class.forName( classMethod.getClazz(), false, testClassLoader );
     ...
     */
    public static Set<ClassMethod> generateFailingTests( final List<Failure> allFailures )
    {
        final Set<ClassMethod> failingMethods = new HashSet<ClassMethod>();

        for ( final Failure failure : allFailures )
        {
            final Description description = failure.getDescription();
            if ( description.isTest() && !isFailureInsideJUnitItself( description ) )
            {
                final ClassMethod classMethod = cutTestClassAndMethod( description );
    ...
    public static Description createSuiteDescription( final Collection<Class<?>> classes )
    {
        return JUnit4Reflector.createRequest( classes.toArray( new Class[classes.size()] ) ).getRunner().getDescription();
    }

    public static boolean isFailureInsideJUnitItself( final Description failure )
    {
     ...
     */
    public static ClassMethod cutTestClassAndMethod( final Description description )
    {
        ...
        if ( name != null )
        {
            ...
            if ( name.endsWith( ")" ) )
            {
                final int classBracket = name.lastIndexOf( '(' );
                if ( classBracket != -1 )
                {
